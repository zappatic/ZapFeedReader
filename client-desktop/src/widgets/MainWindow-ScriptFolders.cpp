    ...
    if (index.isValid())
    {
        ...
        if (mb.clickedButton() == mb.button(QMessageBox::StandardButton::Ok))
        {
            ...
            ZapFR::Engine::Agent::getInstance()->queueRemoveScriptFolder(
                ...
                { QMetaObject::invokeMethod(this, [=, this]() { scriptFolderRemoved(removedSourceID, removedScriptFolderID); }); });
        }
    }
}
                        ...
                        switch (mDialogEditScriptFolder->displayMode())
                        {
                            case DialogEditScriptFolder::DisplayMode::Add:
                            {
                                ZapFR::Engine::Agent::getInstance()->queueAddScriptFolder(sourceID, title, showTotal, showUnread,
                                                                                          ...
                                                                                          { QMetaObject::invokeMethod(this, [=, this]() { scriptFolderAdded(addedSourceID); }); });
                                break;
                            }
                            case DialogEditScriptFolder::DisplayMode::Edit:
                            {
                                ZapFR::Engine::Agent::getInstance()->queueUpdateScriptFolder(
                                    ...
                                    { QMetaObject::invokeMethod(this, [=, this]() { scriptFolderUpdated(updatedSourceID, updatedScriptFolderID); }); });
                                break;
                            }
                        }
                    }
