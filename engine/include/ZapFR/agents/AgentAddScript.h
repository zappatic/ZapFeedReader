...
#ifndef ZAPFR_ENGINE_AGENTADDSCRIPT_H
#define ZAPFR_ENGINE_AGENTADDSCRIPT_H
...
namespace ZapFR
{
    namespace Engine
    {
        class AgentAddScript : public AgentRunnable
        {
          public:
            explicit AgentAddScript(uint64_t sourceID, Script::Type type, const std::string& filename, bool enabled, const std::unordered_set<Script::Event>& events,
                                    const std::optional<std::unordered_set<uint64_t>>& feedIDs, std::function<void(uint64_t)> finishedCallback);
            virtual ~AgentAddScript() = default;
