...
ZapFR::Engine::AgentRemovePostsFromScriptFolder::AgentRemovePostsFromScriptFolder(uint64_t sourceID, uint64_t scriptFolderID,
...
void ZapFR::Engine::AgentRemovePostsFromScriptFolder::run()
{
    ...
    if (source.has_value())
    {
        ...
        for (const auto& [feedID, posts] : feedsWithPostsMap)
        {
            ...
            for (const auto& postID : posts)
            {
                ...
                if (post.has_value())
                {
                    post.value()->unassignFromScriptFolder(mScriptFolderID);
                }
            }
        }
